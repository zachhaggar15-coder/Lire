...
export const metadata: Metadata = {
  ...
  description: "How Sorlio collects, uses, stores, and deletes app and learning data.",
};
...
const sections = [
  ...
  },
  {
    ...
    body: (
      <p>
        Sorlio can be used fully without an account. If you choose to sign in, you do so with Google, and Supabase provides the authentication and account infrastructure. Sorlio never receives your Google password. Sorlio stores an account identifier and your email address so you can see which Google account is signed in; it does not copy your Google profile name, profile picture or other Google account details. Signing in copies the learning stores described in the sync screen to Supabase so they can be merged across your devices. Account and synced data are kept while the account exists and are removed when you delete the account, except for limited records that must be retained for security or legal reasons.
      </p>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <p>
        Sorlio sends text to OpenAI only when an AI-backed feature needs it, such as a requested word explanation, sentence explanation, paraphrase exercise, or natural translation. If natural translation preloading is enabled, Sorlio may request or retrieve a cached translation after an article is opened. The relevant word, sentence, or article text—including text you imported if you use an AI feature on it—may be included in that request. Sorlio does not use this content to advertise to you.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Sorlio uses Supabase for optional accounts, sync, subscription entitlements, feedback, and product records; Google Play for Android subscription billing and purchase verification; Sentry for diagnostics; OpenAI for AI features; Resend for optional beta confirmation email; Upstash for request control and content infrastructure; and Vercel for hosting. These providers process information on Sorlio&apos;s behalf. Sorlio does not sell personal or learning data or share it for behavioural advertising. Information may also be disclosed where required by law or to protect users and the service.
      </p>
    ),
  },
  {
    ...
    body: (
      <p>
        Where UK or European data-protection law applies, optional analytics are processed with your consent; requested accounts, sync, Premium subscriptions, AI, beta, and feedback features are processed to provide the service you ask for; payment, tax, and accounting records are handled where legally required; and limited security and operational records are processed for legitimate interests in keeping Sorlio reliable and preventing abuse. Data is retained only for as long as needed for those purposes, then deleted or anonymised. Sorlio uses HTTPS and access controls, but no online service can guarantee absolute security.
      </p>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Sorlio is an independently operated service and is not guaranteed to remain available indefinitely. The developer may change, suspend, withdraw, or permanently discontinue some or all of Sorlio at any time, including if the project is no longer practical to operate. Where reasonably possible, advance notice will be provided so users can save or remove their information.
        </p>
        <p className="mt-2">
          Discontinuing Sorlio does not remove the developer's obligations to handle retained personal data lawfully. Hosted features such as sync, live news, AI, email, and account recovery may stop when the service ends; information already stored locally may remain on a device until the user clears it or uninstalls the app. Nothing in this section limits rights that cannot legally be excluded.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <p>
        This policy may be updated when Sorlio's features, providers, or legal obligations change. Material changes will be highlighted in the app or changelog where practical. The effective date below identifies the current version.
      </p>
    ),
  },
];
