 ...
 * "Practice the text" and "Listen without text" are real page navigations
 * (/reader/[id]/practice, /reader/[id]/listen) rather than an in-place
 * overlay, so they get their own URL, back-button behaviour, and a page
 * transition instead of just appearing on top of the summary screen.
...
export default function PracticeSection({ text, plan, lookupRate }: PracticeSectionProps) {
  const [completed] = useState(() => isPracticeCompleted(text.id));
  ...
  return (
    <div className="lesson-complete-card-enter mt-4 rounded-card border border-cream-dark bg-cream-card p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {hasExercises && (
          <Link href={`/reader/${text.id}/practice`} className="ligne-pill flex-1 bg-brand py-2.5 text-center text-sm text-cream">
            {completed ? "Practice again" : "Practice the text"}
          </Link>
        )}
        {hasListening && (
          <Link href={`/reader/${text.id}/listen`} className="ligne-pill flex-1 border border-cream-dark bg-cream py-2.5 text-center text-sm text-ink">
            Listen without text
          </Link>
        )}
      </div>
