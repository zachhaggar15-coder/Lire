  ...
  return (
    <section className="rounded-card bg-cream-card p-4 shadow-card">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted">Install Sorlio</p>
          <p className="mt-1 text-sm text-ink-muted">
            Add Sorlio to your home screen for quicker reading sessions and offline access.
          </p>
        </div>
        {promptEvent ? (
          <button type="button" onClick={install} className="shrink-0 rounded-full bg-brand px-4 py-2 shadow-raised text-sm font-semibold text-cream">
            Install
          </button>
        ) : (
