...
--
-- One table, `user_data`, holds every synced localStorage store as a JSONB
-- blob keyed by (user_id, store_key) — store_key is the same string each
-- store already uses as its own localStorage key (see
